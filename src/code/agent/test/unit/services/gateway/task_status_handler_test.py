...
from utils.fc_openapi.fc_client import FCClientException
from services.gateway.handlers.task_status_handler import TaskStatusHandler, _format_task
...
@pytest.fixture
def handler():
    return TaskStatusHandler(MagicMock())
...
class TestHandleGetTask:
    def test_empty_task_id_returns_400(self, app, handler):
        with app.test_request_context("/api/serverless/task/"):
            ...
            assert err["message"] == "taskId is required"

    @patch("services.gateway.handlers.task_status_handler.get_async_task")
    def test_fc_error_returns_500(self, mock_get, app, handler):
        mock_get.side_effect = FCClientException("no creds")
        with app.test_request_context("/api/serverless/task/t1"):
            resp, code = handler.handle_get_task("t1")
            assert code == 500
            err = _err(resp)
            assert err["type"] == "internal_error"
            assert err["message"] == "Failed to query task status"

    @patch("services.gateway.handlers.task_status_handler.get_async_task")
    def test_404_returns_404(self, mock_get, app, handler):
        mock_get.side_effect = FCClientException("not found", status_code=404)
        with app.test_request_context("/api/serverless/task/t1"):
            resp, code = handler.handle_get_task("t1")
            assert code == 404
            err = _err(resp)
    ...
    def test_non_2xx_returns_500(self, mock_get, app, handler):
        mock_get.side_effect = FCClientException("FC returned HTTP 500", status_code=500)
        with app.test_request_context("/api/serverless/task/t1"):
            resp, code = handler.handle_get_task("t1")
            assert code == 500
            err = _err(resp)
    ...
    @patch("services.gateway.handlers.task_status_handler.get_async_task")
    def test_with_detail_includes_payload_and_messages(self, mock_get, app, handler):
        mock_get.return_value = {
            "statusCode": 200,
            "body": {
                "taskId": "tid-1",
        ...
        handler._serverless_api_service.get_status_from_store.return_value = ["m1", "m2"]
        ...
        with app.test_request_context("/api/serverless/task/tid-1?withDetail=true"):
            ...
            handler._serverless_api_service.get_status_from_store.assert_called_once_with("tid-1")

    @patch("services.gateway.handlers.task_status_handler.get_async_task")
    def test_with_detail_payload_none_when_only_fc2_style_invocation_payload(self, mock_get, app, handler):
        """FC 3.0 GetAsyncTask 仅使用 taskPayload；仅有 invocationPayload 时不回填。"""
        mock_get.return_value = {
            "statusCode": 200,
            "body": {
                "taskId": "tid-2",
...
class TestHandleListTasks:
    ...
    def test_fc_error_500(self, mock_list, app, handler):
        mock_list.side_effect = FCClientException("timeout")
        with app.test_request_context("/api/serverless/tasks"):
            resp, code = handler.handle_list_tasks()
            assert code == 500
            err = _err(resp)
            assert err["type"] == "internal_error"
            assert err["message"] == "Failed to list tasks"

    @patch("services.gateway.handlers.task_status_handler.list_async_tasks")
    def test_non_2xx_500(self, mock_list, app, handler):
        mock_list.side_effect = FCClientException("FC returned HTTP 503", status_code=503)
        with app.test_request_context("/api/serverless/tasks"):
            resp, code = handler.handle_list_tasks()
            assert code == 500
