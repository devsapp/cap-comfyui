...
PLUGIN_DIR = (
    ...
    / "custom_nodes"
    / "FunArt-ComfyUI-v0331-Template-Assets"
)
MODULE_PATH = PLUGIN_DIR / "template_asset_resolver.py"
SPEC = importlib.util.spec_from_file_location("v0331_template_asset_resolver", MODULE_PATH)
...
def test_production_manifest_is_pinned_to_v0331_template_release():
    manifest = json.loads(
        (PLUGIN_DIR / "official_template_inputs.json").read_text(encoding="utf-8")
    )
    revision = manifest["source"]["revision"]

    assert manifest["source"]["workflow_templates_version"] == "0.11.41"
    assert revision == "206c7854dc67408d1a571bc59bce8598d7f7e93b"
